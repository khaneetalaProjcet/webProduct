  ...
  <div>
        ...
        <div class="d-card asset">
          <div class="d-flex justify-space-between align-center w-100">
            ...
            <div class="d-flex flex-column justify-space-between h-100 align-end">
              <v-progress-circular v-if="walletLoading" :size="10" color="#fff" indeterminate></v-progress-circular>
          ...
          <div class="action">
            <v-btn
              ...
              >شارژ کیف پول</v-btn
            >
            <!-- <v-btn variant="flat" color="#00603A" class="wallet-btn" @click="depositToCartDialog = true">شارژ کیف
              پول</v-btn> -->
            <v-btn variant="outlined" color="#00603A" class="wallet-btn" @click="withdrawDialog = true">برداشت</v-btn>
          </div>
        </div>
      </v-col>
      ...
      <v-col cols="12">
        ...
        <div class="trade-history">
          <div class="d-flex justify-space-between">
            ...
            <v-combobox label="وضعیت پرداخت" :items="filter" item-title="text" item-value="value" density="comfortable"
              variant="solo" class="transaction-filter" @update:modelValue="filterChange"></v-combobox>
          </div>
            ...
            <v-tabs-window-item value="one" class="transaction-box">
              ...
              <v-data-table :headers="Transactionheaders" :items="DepositeTransactionItems"
                ...
                <template v-slot:item.status="{ item }">
                  <div class="">
                    <v-chip :text="item.status == 'completed'
                      ? 'موفق'
                      : item.status == 'pending'
                        ? 'نامشخص'
                        : 'ناموفق'
    ...
    <v-dialog v-model="depositDialog" width="500">
      <v-card class="cart-Dialog">
        ...
        <v-form ref="form" v-model="isValidcharge" @submit.prevent="behpardakhtDeposit">
          <v-text-field v-model="priceAmount" variant="outlined" color="rgba(135, 104, 36, 1)" density="compact"
            label="مبلغ (تومان)" class="mb-0 mt-2" @input="limitInput"></v-text-field>
          <!-- <div v-if="priceAmount">
            <p class="amount-word">{{ DepositeAmountInWords }}</p>
          </div> -->
          <v-select v-model="cartId" :loading="cartsLoading" label="انتخاب کارت" :items="carts" density="compact"
            variant="outlined" item-title="cardNumber" item-value="id" class="mt-3"
            :rules="[(v) => !!v || 'لطفا کارت خود را انتخاب کنید']"></v-select>
          ...
          <v-btn type="submit" text="شارژ حساب" block color="#9D7E3B" variant="flat" class="submit-cart-btn"
            :disabled="!isValidcharge || !cartId" :loading="depositLoading"></v-btn>
        </v-form>
      </v-card>
    </v-dialog>

    <v-dialog v-model="depositToCartDialog" max-width="600">
      <v-card class="cart-Dialog">
        <h3 class="mb-2">مبلغ مورد نظر را به شماره کارت زیر واریز نمایید و اطلاعات فرم را پر نمایید</h3>
        <ul class="dialog-ul">
          <li>
            واریزی حتما باید از کارت های احراز شده انجام شود.
          </li>
          <li>
            فرمت عکس فیش حتما باید یکی از فرمت های png , jpg , jpeg باشد.
          </li>
        </ul>
        <div class="title my-2">
        ...
        <v-form ref="form" v-model="isValidchargeToCart" @submit.prevent="ToCartSubmit">
          <v-row class="mt-1">
            <v-col cols="12" class="pb-0">
              <v-text-field v-model="toCartDetail.priceAmount" variant="outlined" color="rgba(135, 104, 36, 1)"
                density="compact" label="مبلغ (تومان)" @input="limitInput"></v-text-field>
            </v-col>
            <v-col cols="12" class="pb-0">
              <v-select v-model="toCartDetail.cartId" :loading="cartsLoading" label="انتخاب کارت" :items="carts"
                density="compact" variant="outlined" item-title="cardNumber" item-value="id"
                :rules="[(v) => !!v || 'لطفا کارت خود را انتخاب کنید']"></v-select>
            </v-col>
            <v-col cols="12" class="pb-0">
              <v-text-field v-model="toCartDetail.invoiceId" variant="outlined" color="rgba(135, 104, 36, 1)"
                density="compact" label="شماره پیگیری" :rules="invoiceRule"></v-text-field>
            </v-col>
            <v-col cols="12" md="6">
              <persian-date-picker v-model="toCartDetail.date" placeholder="تاریخ"></persian-date-picker>
            </v-col>
            <v-col cols="12" md="6">
              <persian-date-picker type="time" v-model="toCartDetail.time" placeholder="زمان"></persian-date-picker>
            </v-col>
            <v-col cols="12">
              <v-file-input v-model="toCartDetail.invoiceImage"
              label="بارگذاری عکس"
              accept="image/*"
              counter
              show-size
            ...
            <v-col cols="12" class="pb-0">
              <v-btn type="submit" text="ثبت اطلاعات" block color="#00603a" variant="flat" class="submit-cart-btn"
                :disabled="!isValidchargeToCart || !cartIdToCart" :loading="depositToCartLoading"></v-btn>
            </v-col>
          </v-row>
        </v-form>
      </v-card>
    ...
    <v-dialog v-model="withdrawDialog" width="500">
      <v-card class="cart-Dialog">
        ...
        <v-form ref="form" v-model="isValidPrice" @submit.prevent="withdraw">
          <v-text-field v-model="withdrawAmount" variant="outlined" color="rgba(135, 104, 36, 1)" density="compact"
            label="مبلغ (تومان)" class="mb-0 mt-2 goldconvert-input" :rules="validatePrice" @input="limitWithdrawInput">
            <template #append-inner>
              <span class="allWallet-text" @click="allWallet()">تمام موجودی</span>
            </template>
          </v-text-field>
          <!-- <div v-if="withdrawAmount">
            <p class="amount-word">{{ amountInWords }}</p>
          </div> -->
          <v-select v-model="selectCartId" :loading="cartsLoading" label="انتخاب کارت" :items="carts" density="compact"
            variant="outlined" item-title="cardNumber" item-value="id" class="mt-3"
            :rules="[(v) => !!v || 'لطفا کارت خود را انتخاب کنید']"></v-select>
          <v-btn type="submit" text="برداشت از حساب" block color="#9D7E3B" variant="flat" class="submit-cart-btn"
            :disabled="!isValidPrice" :loading="withdrawLoading"></v-btn>
        </v-form>
      </v-card>
    </v-dialog>
    ...
    <v-dialog max-width="500" v-model="successModal" class="trade-modal">
      <v-card class="success-modal">
        <div class="successModal-content py-5">
          ...
          <img src="/src/assets/images/success-done.jpg" alt="قبت کارت" width="200" height="200" />
          <p>{{ doneText }}</p>
        </div>
      </v-card>
    </v-dialog>
    ...
    <v-dialog max-width="600" v-model="errorDialog" class="trade-modal">
      <v-card class="trade-modal">
        <div class="transferModal-content py-5">
          <h3>خطا</h3>
          <img src="/src/assets/images/error.jpg" alt="خطا" width="200" height="200" />
...
const depositToCartDialog = ref(false);
const withdrawDialog = ref(false);
const priceAmount = ref("");
const priceAmountToCart = ref("");
const withdrawAmount = ref("");
const isValidPrice = ref("");
const depositLoading = ref(false);
const depositToCartLoading = ref(false);
const successMsg = ref("");
const successModal = ref(false);
const errorDialog = ref(false);
const doneText = ref("");
const toCartDetail = ref({
  priceAmount: '',
  cartId: '',
  invoiceId: '',
  invoiceImage: null,
...
const isValidchargeToCart = computed(() => {
  const amount = parseFloat(priceAmount.value.replace(/,/g, ""));
  return amount >= 100000;
});
