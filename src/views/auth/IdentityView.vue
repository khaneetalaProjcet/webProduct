  ...
  <v-card class="auth-card" max-width="360" width="100%">
    ...
    >
      <v-sheet color="surface" class="mt-1">
        <v-text-field
          v-model="identityForm.nationalCode"
          type="text"
          label="کد ملی"
          variant="outlined"
          class="k-textfield"
              ...
              <v-tooltip location="bottom">
                <template v-slot:activator="{ props }">
                  <a
                    href="#"
                    target="_blank"
                    @click.stop.prevent="$router.push({ name: 'Rule' })"
                  >
                    قوانین سایت
                  </a>
                </template>
      ...
      <v-btn
        ...
        class="my-6"
        height="40"
        text="بررسی احراز هویت"
        variant="elevated"
        width="100%"
        color="#B0863B"
        :disabled="!isValid || !siteRule"
        :loading="loading"
      ></v-btn>
      <p class="site-url">
        آدرس رسمی سامانه<span class="k-primary"> www.khanetalaa.ir </span>میباشد
      </p>
    </v-form>
  </v-card>
</template>
...
const validateForm = () => {
  identityForm.value.nationalCode = identityForm.value.nationalCode
    .replace(/\D/g, "")
    .slice(0, 10);
  const isNationalCodeValid = rules.nationalCode.every(
    (rule) => rule(identityForm.value.nationalCode) === true
  );
  const isDateValid =
...
const submitIdentity = async () => {
  try {
    identityForm.value.phoneNumber = JSON.parse(
